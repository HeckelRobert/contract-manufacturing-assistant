...
public sealed class ProposalDraftBuilder(
    IUiTextProvider uiText,
    ContractManufacturingDraftComposer contractManufacturingDraftComposer)
{
    public ProposalDraft Build(ProjectMatch match, UiLanguage language)
    {
        ...
        return new ProposalDraft
        {
            ...
            SourceMode = ProposalSourceMode.HistoricalProject,
        };
    }

    public ProposalDraft BuildFromInquiry(
        CustomerInquiry inquiry,
        UiLanguage language,
        ProjectMatch? referenceProject = null) =>
        contractManufacturingDraftComposer.Compose(inquiry, language, referenceProject);
...
public sealed class ProposalDraft
{
    ...
    public string? CustomerDrawingPath { get; init; }

    public string? ReferenceProjectNumber { get; init; }

    public ProposalSourceMode SourceMode { get; init; } = ProposalSourceMode.HistoricalProject;
}
